...
namespace GUI
{
    partial class MainForm : Form
    {
        ...
        public static ThemeController ThemeManager => themeController;
...
#pragma warning restore CA1810 // Initialize reference type static fields inline
        {
            ...
            themeController.AddTheme(new ThemeWhite());
            themeController.AddTheme(new ThemeDark());
        }
        ...
        public MainForm(string[] args)
        {
            ...
            foreach (string name in themeController.GetThemeNames())
            {
                ToolStripMenuItem i = new ToolStripMenuItem(name);
                i.Click += (sender, e) =>
                {
                    themeController.ApplyTheme(name);
                    Settings.Config.CurrentTheme = SettingsThemeState.FromThemeData(themeController.CurrentTheme);
                    Settings.Save();
                };
                themeToolStripMenuItem.DropDownItems.Add(i);
            }

            themeToolStripMenuItem.DropDownItems.Add(new ToolStripSeparator());
            ToolStripMenuItem toolStripThemeEditor = new ToolStripMenuItem("Theme Editor");

#pragma warning disable CA2000 // Dispose objects before losing scope
            ThemeEditorDialog dialog = new ThemeEditorDialog();
#pragma warning restore CA2000 // Dispose objects before losing scope
            toolStripThemeEditor.Click += (sender, e) =>
            {
                dialog.ShowDialog(this);
            };
            themeToolStripMenuItem.DropDownItems.Add(toolStripThemeEditor);

            themeController.RegisterControl(this);
            themeController.RegisterControl(searchForm);

            themeController.RegisterControl(tabContextMenuStrip);
            themeController.RegisterControl(vpkContextMenu);
            themeController.RegisterControl(vpkEditingContextMenu);

            mainTabs.ControlAdded += mainTabs_OnControlAdded;
            mainTabs.ControlRemoved += mainTabs_OnControlRemoved;

            if (Settings.Config.CurrentTheme == null)
                themeController.ApplyTheme("Dark");
            else
                themeController.ApplyTheme(Settings.Config.CurrentTheme.ToThemeData());
        }

        private void mainTabs_OnControlAdded(object sender, ControlEventArgs e)
        {
            themeController.RegisterControl(e.Control);
        }

        private void mainTabs_OnControlRemoved(object sender, ControlEventArgs e)
        {
            themeController.UnregisterControl(e.Control);
        }
        ...
        private void OnAboutItemClick(object sender, EventArgs e)
        {
            ...
            themeController.RegisterControl(form);
        ...
        public Task<TabPage> OpenFile(VrfGuiContext vrfGuiContext, PackageEntry file, TreeViewWithSearchResults packageTreeView = null)
        {
            ...
            // It is fine to register multiple times as it will just update the style if it is added already before.
            themeController.RegisterControl(tab);
