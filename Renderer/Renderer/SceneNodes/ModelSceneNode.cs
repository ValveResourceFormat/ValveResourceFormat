...
namespace ValveResourceFormat.Renderer.SceneNodes
{
    ...
    public class ModelSceneNode : MeshCollectionNode
    {
            ...
            : base(scene)
        {
            ...
            TwistConstraints = ParseTwistConstraints(model);
        }
...
#pragma warning restore CA1024 // Use properties where appropriate
        ...
        /// <summary>
        /// Applies tilt-twist constraints to the animated skeleton. Called after animation controller updates.
        /// Override this method to implement constraint logic or customize debug visualization.
        /// </summary>
        protected virtual void ApplyConstraints(Scene.UpdateContext context)
        {
            ...
            {
                // Evaluate constraint and display debug info
                var evalResult = EvaluateTiltTwistConstraint(constraint, skeleton, AnimationController.Pose);

                context.TextRenderer.AddTextRelative($"Constraint: {constraint.Name}", 0.02f, yOffset, 11f, Color32.White, context.Camera);
                yOffset += 0.02f;

                // Display target info
                foreach (var target in constraint.Targets)
                {
                    var boneName = StringToken.GetKnownString(target.BoneHash);
                    var bone = skeleton[target.BoneHash];

                    if (bone != null)
                    {
                        context.TextRenderer.AddTextRelative($"  Target: {boneName} (w={target.Weight:F2})", 0.02f, yOffset, 11f, new Color32(0f, 1f, 1f, 1f), context.Camera);
                    }
                    else
                    {
                        context.TextRenderer.AddTextRelative($"  Target: {boneName} NOT FOUND", 0.02f, yOffset, 11f, Color32.Red, context.Camera);
                    }
                    yOffset += 0.02f;
                }

                // Display twist angle
                context.TextRenderer.AddTextRelative($"  Twist: {evalResult.TwistAngleDegrees:F1}deg", 0.02f, yOffset, 11f, Color32.Yellow, context.Camera);
                yOffset += 0.02f;

                // Display slave info
                foreach (var slave in constraint.Slaves)
                {
                    var boneName = StringToken.GetKnownString(slave.BoneHash);
                    var bone = skeleton[slave.BoneHash];

                    if (bone != null)
                    {
                        context.TextRenderer.AddTextRelative($"  Slave: {boneName} (w={slave.Weight:F2})", 0.02f, yOffset, 11f, Color32.Green, context.Camera);
                    }
                    else
                    {
                        context.TextRenderer.AddTextRelative($"  Slave: {boneName} NOT FOUND", 0.02f, yOffset, 11f, Color32.Red, context.Camera);
                    }
                    yOffset += 0.02f;
                }

                yOffset += 0.01f; // Space between constraints
            }
        }
        ...
        /// <summary>
        /// Evaluates a tilt-twist constraint by computing the twist rotation from target bones.
        /// Applies the twist to slave bones and updates the pose hierarchy.
        /// </summary>
        ...
        {
            var twistAxis = GetAxisVector(constraint.TargetAxis);
            var accumulatedTwist = Quaternion.Identity;
            var totalWeight = 0f;

            // Get weighted average twist from all targets
            foreach (var target in constraint.Targets)
            {
                var bone = skeleton[target.BoneHash];
                if (bone == null || target.Weight <= 0f)
                {
                    continue;
                }
                ...
                // Remove the bind pose to get only the animated delta
                ...
                // Decompose rotation into twist (around axis) and swing (perpendicular)
                var twist = DecomposeTwistRotation(deltaRotation, twistAxis);

                // Accumulate twist using weighted slerp (proper quaternion averaging)
                if (totalWeight == 0f)
                {
            ...
            foreach (var slave in constraint.Slaves)
            {
                ...
                // Apply weighted twist to the local rotation directly
                var weightedTwist = Quaternion.Slerp(Quaternion.Identity, accumulatedTwist, slave.Weight);
                var newLocalRotation = Quaternion.Multiply(weightedTwist, localRotation);

                // Apply the new local rotation and rebuild hierarchy
                ApplyLocalRotationAndRebuildChildren(bone, newLocalRotation, pose);
            }
        ...
        /// <summary>
        /// Extracts the local rotation of a bone relative to its parent from world-space pose.
        /// </summary>
        private static Quaternion GetLocalRotation(Bone bone, Span<Matrix4x4> pose)
        {
            ...
            if (bone.Parent != null)
            {
                ...
                return Quaternion.Multiply(Quaternion.Inverse(parentRotation), worldRotation);
            }
        ...
        /// <summary>
        /// Applies a new local rotation to a bone and recursively updates world transforms for all children.
        /// </summary>
        private static void ApplyLocalRotationAndRebuildChildren(Bone bone, Quaternion newLocalRotation, Span<Matrix4x4> pose)
        {
            // Get parent world transform
            var parentTransform = bone.Parent != null ? pose[bone.Parent.Index] : Matrix4x4.Identity;

            // Extract current local translation and scale from world transform
            if (bone.Parent != null)
            {
                // Transform world position to parent local space to get local translation
                Matrix4x4.Decompose(pose[bone.Index], out var worldScale, out _, out var worldTranslation);
                Matrix4x4.Decompose(parentTransform, out _, out _, out var parentTranslation);
                
                var localTranslation = Vector3.Transform(worldTranslation - parentTranslation, Matrix4x4.CreateFromQuaternion(Quaternion.Inverse(GetRotation(parentTransform))));

                // Build new local transform
                var localTransform = Matrix4x4.CreateScale(worldScale)
                    * Matrix4x4.CreateFromQuaternion(newLocalRotation)
                    * Matrix4x4.CreateTranslation(localTranslation);

                // Compute new world transform
                pose[bone.Index] = localTransform * parentTransform;
            }
            else
            {
                // Root bone - just update rotation in world space
                Matrix4x4.Decompose(pose[bone.Index], out var scale, out _, out var translation);
            ...
            {
                RebuildChildWorldTransform(child, pose);
            }
        }

        /// <summary>
        /// Recursively rebuilds world transforms for a bone and its children based on parent's updated transform.
        /// </summary>
        private static void RebuildChildWorldTransform(Bone bone, Span<Matrix4x4> pose)
        {
            var parentTransform = bone.Parent != null ? pose[bone.Parent.Index] : Matrix4x4.Identity;

            // Get the bone's current local rotation (preserve animation)
            var localRotation = GetLocalRotation(bone, pose);

            // Extract local translation from world space
            Matrix4x4.Decompose(pose[bone.Index], out var worldScale, out _, out var worldTranslation);
            
            Vector3 localTranslation;
            if (bone.Parent != null)
            {
                Matrix4x4.Decompose(parentTransform, out _, out var parentRotation, out var parentTranslation);
                var relativeTranslation = worldTranslation - parentTranslation;
                localTranslation = Vector3.Transform(relativeTranslation, Matrix4x4.CreateFromQuaternion(Quaternion.Inverse(parentRotation)));
            }
            else
            {
            ...
            // Rebuild local transform with preserved animation
            var localTransform = Matrix4x4.CreateScale(worldScale)
        ...
        /// <summary>
        /// Decomposes a quaternion into its twist component around the given axis.
        /// </summary>
        ...
        /// <summary>
        /// Converts axis index (0=X, 1=Y, 2=Z) to unit vector.
        /// </summary>
