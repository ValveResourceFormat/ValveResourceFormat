...
internal sealed class SoundEventHLVRAmbientMultiVsnd : SoundEvent
{
    ...
    protected override void DoStart()
    {
        ...
        StartChildren(childDefinitions, applyLayer);
    }

    private void ApplyLayer(SoundEvent child, int i)
    {
        child.Position = Position;
        child.VolumeOverride = startBaseVolume * layers[i].Weight;
    }
    ...
    internal override void Prewarm(int depth)
    {
        ...
        // Bakes the definition's own delay, same as a first start without a DelayOverride
        PrewarmChildren(Definition.ChildDefinitions ??= BuildLayerDefinitions(Definition.Delay), depth);
    }
