...
/// predictor state), so <see cref="PcmDecoder"/> streams them through in chunks.
/// </summary>
internal static class MsAdpcmDecoder
{
    ...
    /// <summary>
    /// Decodes a single MS ADPCM block into interleaved float samples, returning the sample count.
    /// The caller guarantees 1-2 channels and a block at least 7 bytes per channel long.
    /// </summary>
    public static int DecodeBlock(ReadOnlySpan<byte> block, int channels, Span<float> output)
    {
        var outputIndex = 0;
        var pos = 0;
        ...
        {
            var predictor = Math.Min((int)block[pos++], 6);
        ...
        for (var ch = 0; ch < channels; ch++)
        {
            delta[ch] = BitConverter.ToInt16(block[pos..]);
            pos += 2;
        }

        for (var ch = 0; ch < channels; ch++)
        {
            sample1[ch] = BitConverter.ToInt16(block[pos..]);
            pos += 2;
        }

        for (var ch = 0; ch < channels; ch++)
        {
            sample2[ch] = BitConverter.ToInt16(block[pos..]);
            pos += 2;
        }
