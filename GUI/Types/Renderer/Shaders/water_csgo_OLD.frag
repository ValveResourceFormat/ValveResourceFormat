...
void main()
{
    ...
    if(do_wave_approximation)
    {
        ...
        normal = normalize( vec3(flat_normal, 1.0));

        normal = DecodeHemiOctahedronNormal( texture(g_tWavesNormalHeight, local_pos.xy / 200).rg ); //  - water_height(local_pos.xy)  * 10 );

        normal.xy *= 10;
        normal = normalize(normal);

        if(vColorBlendValues.z > 0.5)
        {
            outputColor.rgb = texture(g_tDebris, vTexCoordOut * 10).rgb;
            return;
        }


        //DecodeHemiOctahedronNormal( DecodeHemiOctahedronNormal( texture(g_tWavesNormalHeight, local_pos).  );
...
float water_height(vec2 coords)
{
    ...
    water_height = 1 - texture(g_tWavesNormalHeight, coords / 200).b;
