...
internal sealed class AnimGraph1Builder : IDisposable
{
    ...
    /// <summary>Whether the graph has any parameter consumer, set once a build has run.</summary>
    public bool HasParameterConsumers => parameterConsumers.Count > 0;

    /// <summary>Receives diagnostics about nodes the schema could not be read cleanly from.</summary>
    public IProgress<string>? ProgressReporter { get; set; }

    /// <summary>Creates a builder over one animation graph.</summary>
    /// <param name="data">The graph, compiled or in the editor schema.</param>
    /// <param name="loader">Loads the model the graph binds to, for bone and sequence names.</param>
    public AnimGraph1Builder(KVObject data, IFileLoader loader)
    {
        animGraphData = data;
        fileLoader = loader;
        modelInfo = new AnimGraphModelInfo(loader, LoadModel);
    }
    ...
    /// <summary>Releases the model resource the graph resolved names against.</summary>
    public void Dispose()
    {
    ...
    private static void AddScalarRows(Node node, KVObject source, Func<string, bool> skip, int maxRows = int.MaxValue, Func<string, KVObject, string?>? renderRow = null)
    {
        ...
        foreach (var (key, child) in source)
        {
            ...
            if (text == null)
            {
                ...
                var value = KVGraphNode.StringifyValue(child);
        ...
        HashSet<string> usedGroupPaths)
    {
        foreach (var pair in nodePairs)
        {
            ...
            AddEditorNodes(document, childNodes, childPath, editorNodes, editorData, outputNames, usedGroupPaths);
        }
    }
    ...
    private void BuildEditorGraph(GraphDocument document)
    {
        ...
        {
            document.AddNode(new Node(null)
            {
        ...
        AddEditorNodes(document, rootNodes, groupPath: null, editorNodes, editorData, outputNames, usedGroupPaths);
        ...
        foreach (var (id, value) in editorData)
        {
            ...
            {
                ConnectEditorStates(document, value.GetArray("m_states"), editorNodes, paramNames, ConnectConnection, ConnectSource);
            }
        }
    }
    ...
    /// </summary>
    private static void ConnectEditorStates(
        GraphDocument document,
        ...
        Action<long, long, string> connectSource)
    {
        ...
        foreach (var state in states)
        {
            ...
            foreach (var transition in state.GetArray("m_transitions"))
            {
                ...
                AnimGraphHues.ConnectTransition(document, srcNode, destNode, label);
            }
        }
    }
    ...
    private void LoadParameters(GraphDocument document)
    {
    ...
    private void LoadTags(GraphDocument document)
    {
        tags.Clear();
    ...
    private void LoadComponents(GraphDocument document)
    {
        components.Clear();
    ...
    private void CreateGraph(GraphDocument document)
    {
        ...
        for (int i = 0; i < compiledNodes.Count; i++)
        {
            ...
            foreach (var (childIdx, label) in connections)
            {
                ...
                document.Connect(outputSocket, inputSocket);
            }
        }
    }
    ...
    private void AddParameterAndTagNodes(GraphDocument document)
    {
            ...
            foreach (var p in ordered)
            {
                if (DrawParameterWires && parameterConsumers.TryGetValue(p, out var consumers))
                {
                    ...
                    foreach (var consumer in consumers)
                    {
                        ...
                        document.Connect(output, input, dashed: true);
                    }
                }
                else
                {
