...
namespace ValveResourceFormat.ResourceTypes.RubikonPhysics.Softbody
{
    ...
    /// its plane.
    /// </summary>
    internal readonly record struct PlanarizeSample(int Node, Vector3 Local, Vector3 Normal, float Offset,
        float Gap, float Radius);

    /// <summary>A recovered planarized shape. Its two end caps coincide when the shape is a sphere.</summary>
    internal readonly record struct CapsuleFit(Vector3 C0, float R0, Vector3 C1, float R1);

    /// <summary>
    /// Recovers the capsules, spheres and boxes a control parent's planarized collision planes were compiled from.
    /// </summary>
    internal static class PlanarizedShapeFitter
    {
        ...
        /// </summary>
        internal static (Quaternion Rotation, Vector3 Min, Vector3 Max)? FitOrientedPlanarizedBox(List<PlanarizeSample> samples)
        {
        ...
        /// nodes they own.
        /// </summary>
        internal static (Vector3 Point0, Vector3 Point1) PlanarizedAxis(CapsuleFit fit,
        ...
        /// </summary>
        internal static List<(CapsuleFit Fit, List<int> Members)>? FitPlanarizedShapes(List<PlanarizeSample> samples)
        {
            if (samples.Count < 3)
            {
                return null;
