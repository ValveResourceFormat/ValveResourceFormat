...
namespace ValveResourceFormat.ResourceTypes.RubikonPhysics.Softbody
{
    ...
    public sealed partial class FeModel
    {
        ...
        // proxy DMX face order is not - the recompile emits no basis at all for those nodes (183 entries
        // across three Deadlock models, plus a 156-model Dota bucket). Recovering it needs a new datum
        // carrying the original face order, or a search over the orderings.
        ...
        // Records one more occurrence of an unordered node pair.
        static void CountPair(Dictionary<(int, int), int> counts, int a, int b)
        {
            if (a < 0 || b < 0)
            {
                return;
        ...
        static Dictionary<(int, int), int> ChainGeneratedSpans(List<BoneChain> chains)
        {
            ...
            void Generate(int a, int b) => CountPair(generated, a, b);
...
#pragma warning disable CS1591
        ...
        // TODO: unread. MakeClothParams deliberately takes the shear from the rod relaxation factors
        // instead, because this field tracks m_flDefaultSurfaceStretch whatever the shear is.
        ...
        // TODO: unread, and the largest single round-trip loss left in the cloth export - m_nStaticNodeFlags
        // differs on 467 model rows across the Dota, Deadlock and CS2 ledgers, and is non-zero on 131 of the
        // 148 cloth models the other seven games ship. DynamicNodeFlags below is the sibling that IS
        // recovered (its bits drive the ClothParams switches in MakeClothParams); the static half has no
        // reader at all. Find which authored keys its bits carry and emit them the same way.
        ...
        // TODO: nothing reads the scalars below. CModelDocClothParams registers 36 authorable keys and
        // MakeClothParams emits all 36; none of these has a counterpart there, and the Dota/CS2/Deadlock
        // round trip never reports them, so the compiler re-derives them. They are NOT always zero: a
        // census over the other seven Source 2 games found them non-zero on all 148 cloth models Half-Life
        // Alyx, Underlords, Aperture Desk Job and Artifact Classic ship. Decide per key whether that is
        // authored data an older era could carry or a derived value, before reading or removing any.
...
#pragma warning restore CS1591
        ...
        // TODO: parsed, never read, and no shipped model found carrying any - empty across all ten Source 2 games.
        // Kept so AssertAllKeysAccountedFor stays honest and the element shape stays recorded; wire it up if a
        // model ever ships one.
        ...
        // TODO: parsed, never read. Real data exists for it: 23 model rows differ on m_DynKinLinks in the
        // Dota/Deadlock/CS2 ledgers. Recovering it means finding the ModelDoc construct that authors it and
        // emitting that alongside the rest of the cloth.
        ...
        // TODO: parsed, never read. Real data exists for it: 31 ledger rows, plus 3 of the 148 cloth models in the
        // other seven games. Recovering it means finding the ModelDoc construct that authors it and emitting that
        // alongside the rest of the cloth.
        ...
        /// model.
        /// </summary>
        // TODO: parsed, never read. Real data exists for it: 6 of Half-Life Alyx's 49 cloth models. Recovering it
        // means finding the ModelDoc construct that authors it and emitting that alongside the rest of the cloth.
        ...
        // TODO: no exported construct reproduces these. Every model with an ImportedCloth carries them
        // (119 in the Dota corpus) plus Deadlock's fencer, which has none. 38 node-field candidates were
        // probed and rejected; the open lead is a cloth_suspenders vertex paint pairing outer and inner
        // sheet vertices, which needs a painted proxy DMX to test, not a node field.
        ...
        /// </summary>
        // TODO: parsed, never read. Real data exists for it: 16 cloth models across Half-Life Alyx, Underlords and
        // Artifact Classic. Recovering it means finding the ModelDoc construct that authors it and emitting that
        // alongside the rest of the cloth.
        ...
        public FeModel(KVObject data)
        {
            ...
            // TODO: the self-rods dropped below (nNode = [i, i]) are not authorable on the current
            // compiler - every registered attribute of every instantiable cloth construct was compile-
            // tested against them and rejected, and ClothSpring refuses equal endpoints outright. An older
            // compiler emitted them and 559 of the 2517 Dota models still ship them, 13,421 entries in
            // total, which is the entire rod deficit on 151 of those models.
