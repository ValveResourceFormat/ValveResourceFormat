...
namespace ValveResourceFormat.IO
{
    ...
    internal class HammerMeshBuilder
    {
        ...
        public void AddPhysMesh(MeshDescriptor desc, PhysAggregateData phys, HashSet<string> proceduralPhysMaterialsToExtract, Vector3 positionOffset = new Vector3(), string? materialOverride = null)
        {
            var attributes = phys.CollisionAttributes[desc.CollisionAttributeIndex];
            var tags = attributes.GetArray<string>("m_InteractAsStrings") ?? attributes.GetArray<string>("m_PhysicsTagStrings");
            var group = attributes.GetStringProperty("m_CollisionGroupString");
            ...
            //still ignore default for now since were not separating vmap mesh collision from vmdl
            if (group == "Default")
            {
                return;
            }
