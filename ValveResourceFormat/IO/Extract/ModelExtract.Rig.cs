...
partial class ModelExtract
{
    ...
    /// </summary>
    private static string GetBoneReferenceName(KVObject? source, string key)
        => source?.ContainsKey(key) == true
            ? source.GetSubCollection(key).GetStringProperty("m_Name", string.Empty)
            : string.Empty;

    private static KVObject MakeNamedReference(KVObject? source, string key)
    {
    ...
    private static KVObject MakeAnimParamReference(KVObject? source, string key)
    {
        var reference = KVObject.Collection();
